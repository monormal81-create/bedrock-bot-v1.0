...
from pixel_art.generator import generate_all_textures, get_preview_texture, scale_texture_for_preview
...
def kb_confirm():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("👁️ معاينة الشكل", callback_data="confirm_preview")],
        [InlineKeyboardButton("✅ أنشئ مباشرة", callback_data="confirm_yes"),
...
async def _show_preview_message(query, uid, d):
    """يرسل صورة المعاينة"""
    try:
        preview_bytes = get_preview_texture(d)
        if not preview_bytes:
            # لا توجد صورة — اذهب للبناء مباشرة
            return await _build_and_send(query, uid, d)

        mod_type = d.get("mod_type","item")
        color = d.get("color1","gray")
        name = d.get("display_name") or d.get("display_prefix","المود")

        type_emoji = {"item":"⚔️","food":"🍖","armor":"🛡️","entity":"🐉"}.get(mod_type,"📦")

        caption = (
            f"{type_emoji} معاينة: {name}\n"
            f"اللون: {color}\n\n"
            f"هل تقبل هذا الشكل؟"
        )
        ...
        await query.message.reply_photo(
            photo=_io.BytesIO(preview_bytes),
            caption=caption,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ موافق — أنشئ المود!", callback_data="preview_approve")],
                [InlineKeyboardButton("🎨 غيّر اللون", callback_data="preview_change_color")],
                [InlineKeyboardButton("⬇️ أنشئ بدون معاينة", callback_data="preview_skip")],
            ])
        )
        await query.edit_message_text(f"👆 معاينة {name}")
        return S_PREVIEW

    except Exception as e:
        # إذا فشلت المعاينة — اذهب للبناء مباشرة
        return await _build_and_send(query, uid, d)


async def _build_and_send(query, uid, d):
    """يبني المود ويرسله"""
    await query.message.reply_text("⚙️ جارٍ بناء المود...")
    mod_type = d.get("mod_type", "item")
    ...
    try:
        ...
        try:
            files.update(generate_all_textures(d))
        ...
        if d.get("in_bundle"):
            ...
            await query.message.reply_text(
                f"✅ تم إضافة {d.get('display_name','مود')} للحزمة! ({count} مودات)\n\nأضف مزيداً أو أنشئ الحزمة:",
                reply_markup=InlineKeyboardMarkup([
        ...
        os.remove(mcpack_path)
        await query.message.reply_text("✅ تم إنشاء المود بنجاح!\nتم حفظه — أرسل /mymods لعرض مودات")
