...
SWORD = {
  ...
  "minecraft:item": {
    "description": {"identifier": "{{NAMESPACE}}:{{ITEM_ID}}", "category": "Equipment"},
    "components": {
...
AXE = {
  ...
  "minecraft:item": {
    ...
    "components": {
      ...
      "minecraft:durability": {"max_durability": "{{DURABILITY}}"},
      "minecraft:damage": "{{DAMAGE}}",
      "minecraft:enchantable": {"value": 10, "slot": "axe"},
      "minecraft:icon": {"texture": "{{ITEM_ID}}"},
      "minecraft:display_name": {"value": "{{DISPLAY_NAME}}"}
    }
  }
  ...
  "minecraft:item": {
    ...
    "components": {
      ...
      "minecraft:shooter": {
        ...
        "max_draw_duration": "{{DRAW_DURATION}}",
...
FOOD = {
  "format_version": "1.20.0",
  "minecraft:item": {
    "description": {"identifier": "{{NAMESPACE}}:{{ITEM_ID}}", "category": "Items"},
    "components": {
...
ITEM_TEXTURE = {
  ...
  "texture_data": {
    "{{ITEM_ID}}": {"textures": "textures/items/{{ITEM_ID}}"}
  }
}
...
ARMOR_PIECE = {
  ...
  "minecraft:item": {
    ...
    "components": {
      "minecraft:max_stack_size": 1,
      "minecraft:durability": {"max_durability": "{{DURABILITY}}"},
      "minecraft:armor": {"protection": "{{PROTECTION}}"},
      "minecraft:enchantable": {"value": 10, "slot": "armor_{{SLOT}}"},
...
HOSTILE = {
  ...
  "minecraft:entity": {
    ...
    "components": {
      "minecraft:type_family": {"family": ["{{ENTITY_ID}}", "monster"]},
      ...
      "minecraft:behavior.nearest_attackable_target": {
        "priority": 2,
        "within_radius": "{{ATTACK_RADIUS}}",
        "entity_types": [{"filters": {"test": "is_family", "subject": "other", "value": "player"}, "max_dist": "{{ATTACK_RADIUS}}"}]
      },
      ...
      "minecraft:nameable": {},
      "minecraft:loot": {"table": "loot_tables/entities/{{ENTITY_ID}}.json"},
...
PASSIVE = {
  ...
  "minecraft:entity": {
    ...
    "components": {
      "minecraft:type_family": {"family": ["{{ENTITY_ID}}", "animal"]},
      "minecraft:health": {"value": "{{HEALTH}}", "max": "{{HEALTH}}"},
      "minecraft:movement": {"value": "{{SPEED}}"},
      "minecraft:navigation.walk": {"can_path_over_water": False},
      "minecraft:behavior.random_stroll": {"priority": 6, "speed_multiplier": 1.0},
...
COMPANION = {
  ...
  "minecraft:entity": {
    ...
    "component_groups": {
      "{{NAMESPACE}}:tamed": {
        "minecraft:behavior.follow_owner": {"priority": 2, "speed_multiplier": 1.2, "start_distance": 10, "stop_distance": 2},
    ...
    "components": {
      "minecraft:type_family": {"family": ["{{ENTITY_ID}}"]},
      "minecraft:health": {"value": "{{HEALTH}}", "max": "{{HEALTH}}"},
      "minecraft:attack": {"damage": "{{DAMAGE}}"},
      "minecraft:movement": {"value": "{{SPEED}}"},
      "minecraft:tameable": {
        "probability": "{{TAME_CHANCE}}",
        "tame_items": "{{TAME_ITEM}}",
        "tame_event": {"event": "{{NAMESPACE}}:on_tame", "target": "self"}
      },
    ...
    "events": {
      "{{NAMESPACE}}:on_tame": {
        "remove": {"component_groups": ["{{NAMESPACE}}:wild"]},
        "add": {"component_groups": ["{{NAMESPACE}}:tamed"]}
      }
    }
  }
}
...
ENTITY_CLIENT = {
  ...
  "minecraft:client_entity": {
    "description": {
      ...
      "textures": {"default": "textures/entity/{{ENTITY_ID}}"},
      "geometry": {"default": "geometry.{{ENTITY_ID}}"},
      "render_controllers": ["controller.render.{{ENTITY_ID}}"],
      "spawn_egg": {
        "base_color": "{{EGG_COLOR_1}}",
        "overlay_color": "{{EGG_COLOR_2}}"
      }
    }
  }
}
...
RENDER_CONTROLLER = {
  "format_version": "1.10.0",
  "render_controllers": {
    "controller.render.{{ENTITY_ID}}": {
      "geometry": "geometry.{{ENTITY_ID}}",
...
RECIPE_SHAPED = {
  ...
  "minecraft:recipe_shaped": {
    "description": {"identifier": "{{NAMESPACE}}:recipe_{{ITEM_ID}}"},
    "tags": ["crafting_table"],
    "pattern": "{{PATTERN}}",
    "key": "{{KEY}}",
    "result": {"item": "{{NAMESPACE}}:{{ITEM_ID}}", "count": "{{RESULT_COUNT}}"}
  }
}
...
RECIPE_FURNACE = {
  ...
  "minecraft:recipe_furnace": {
    "description": {"identifier": "{{NAMESPACE}}:furnace_{{ITEM_ID}}"},
...
LOOT_TABLE = {
  "pools": [
    {
      "rolls": {"min": "{{MIN_ROLLS}}", "max": "{{MAX_ROLLS}}"},
